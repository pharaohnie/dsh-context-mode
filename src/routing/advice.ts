...
## 沙箱契约（ctx_execute*）
- code 是 async function body：顶层 await/return 可用；**无 require/静态 import**。
- 读宿主文件系统：const fs = await import("node:fs")（dynamic import 可用）；管道/命令行 → language:"shell" 或 bash 工具。
...
- ctx_execute* 的 code 无 require/静态 import；读宿主文件用 await import("node:fs")；FILE_SRC=文件完整内容字符串，勿再读文件；管道/命令行走 language:"shell"。
